...
<%
	...
	SQLConn.sortAssignments(currentSession, "dueDate", kindOfSort);
	...
	if(kindOfSort.equals("asc")) {
		...
		for(int i = 0; i<assignments.size()-1; i++) {
			listOfDates.add(assignments.get(i).getDueDate());
		}
		...
		for(int i = 0; i<listOfDates.size(); i++) {
			for(int j = 0; j<assignments.size(); j++) {
				if(listOfDates.get(i).equals(assignments.get(j).getDueDate())) {
