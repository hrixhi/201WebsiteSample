...
<%
	...
	SQLConn.sortAssignments(currentSession, "gradePercentage", kindOfSort);
	...
	if(kindOfSort.equals("asc")) {
		...
		ArrayList<String> listOfGP = new ArrayList<String>();	
		ArrayList<Assignment> sortedArray = new ArrayList<Assignment>();
		
		
		for(int i = 0; i<assignments.size(); i++) {
			if(assignments.get(i).getGradePercentage()!=null) {
				listOfGP.add(assignments.get(i).getGradePercentage());
			}
			else {
				listOfGP.add("10");
			}
		...
		for(int i = 0; i<listOfGP.size(); i++) {
			for(int j = 0; j<assignments.size()-1; j++) {
				if(temp.get(j) == 1) { continue; }
				if(listOfGP.get(i).equals(assignments.get(j).getGradePercentage())) {
					sortedArray.add(assignments.get(j));  temp.set(j, 1); break;
				}
			}
		}
		...
		//CompareTitle comp = new CompareTitle();
		for(int i = 0; i<assignments.size(); i++) {
			if(assignments.get(i).getGradePercentage()!=null) {
				listOfDates.add(assignments.get(i).getGradePercentage());
			}
			else {
				listOfDates.add("10");
			}
	...
	ArrayList<Integer> temp = new ArrayList<Integer>();
		...
		for(int i = 0; i<listOfDates.size(); i++) {
			for(int j = 0; j<assignments.size()-1; j++) {
				if(temp.get(j) == 1) { continue; }
				if(listOfDates.get(i).equals(assignments.get(j).getGradePercentage())) {
					sortedArray.add(assignments.get(j));  temp.set(j, 1); break;
				}
			}
		}
