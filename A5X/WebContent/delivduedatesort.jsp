...
<%
	...
	// MAIN WORK !!!
		...
		// SQL SORTING
		SQLConn.sortDeliverables(currentSession, "dueDate", kindOfSort);
	...
	if(kindOfSort.equals("asc")) {
		...
		for(int ix = 0; ix<delivs.size(); ix++) {
			listOfDates.add(delivs.get(ix).getDueDate());
		}
		...
		for(int ix = 0; ix<listOfDates.size(); ix++) {
			for(int j = 0; j<delivs.size(); j++) {
				if(listOfDates.get(ix).equals(delivs.get(j).getDueDate())) {
					sortedArray.add(delivs.get(j)); 
				}
			}
		}
