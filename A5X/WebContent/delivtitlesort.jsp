...
<%
	...
	// MAIN WORK !!!
		...
		// SQL SORTING
		SQLConn.sortDeliverables(currentSession, "title", kindOfSort);
	...
	if(assignments.get(x).getDeliverables().size() == 0) {
		...
		i = x;
		delivs = assignments.get(i).getDeliverables();
	}
	...
	if(kindOfSort.equals("asc")) {
		...
		for(int ix = 0; ix<delivs.size(); ix++) {
			titles.add(delivs.get(ix).getTitle());
		}
		...
		for(int ix = 0; ix<titles.size(); ix++) {
			for(int j = 0; j<delivs.size(); j++) {
				if(temp.get(j) == 1) { continue; }
				if(titles.get(ix).equals(delivs.get(j).getTitle())) {
...
<table border="2" cellpadding="5">
	<!--  This is the table that is going to be used for the final project  -->

	<tr>
		<td colspan="3" align="center"><a
			href="<%=assignments.get(i).getUrl()%>"> <%=assignments.get(i).getTitle()%>
		</a>
		<hr> <% for(int k = 0; k<assignments.get(i).getFiles().size(); k++) {%>
			<% if(! assignments.get(i).getFiles().get(k).getTitle().equals("")) {%>

			<a href="<%=assignments.get(i).getFiles().get(k).getUrl() %>"> <%=assignments.get(i).getFiles().get(k).getTitle()%>
				<br /></a> <% } %> <% if(k!=assignments.get(i).getFiles().size()-1) { %>
			<hr> <%}%> <% } %></td>
	</tr>
	...
	<tr>
		...
		<td><%=assignments.get(i).getDeliverables().get(d).getTitle()%><br>
			...
			<a
			href="<%=assignments.get(i).getDeliverables().get(d).getFiles().get(f).getUrl() %>">
				<%=assignments.get(i).getDeliverables().get(d).getFiles().get(f).getTitle()%>
				<br />
		</a> <% } %> <% if(f!=assignments.get(i).getDeliverables().get(d).getFiles().size()-1) { %>
			<hr> <%}%> <% } %></td>
